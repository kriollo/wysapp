...
abstract class Controllers {
    ...
    */
    ...
    private function knowVisitorPermissions() {
      ...
      if ($this->is_logged){
        if ((new Model\Users)->validar_cambio_pass() != false && $this->controllerConfig['valida_pass_vencida'] == true) {
          // $this->template->display('users/perfil_user',array(
          //     'db_campus' => (new Model\Users)->getCampusUser('this'),
          //     'reset_pass' => true
          // ));
          ...
          if ($this->controllerConfig['only_admin'] && $this->user['rol'] != 1 ){
            Helper\Functions::redir($config['build']['url']. 'error?e=403');
          }
          # acceso opcion sólo si usuario tiene permiso en perfil
          if ($this->controllerConfig['access_menu'] != false){
            if ($this->controllerConfig['access_menu']['valida_acceso']){
              
              $flat = false;
              foreach ($this->menu_user as $key => $value) {
                if ($value['id_menu'] == $this->controllerConfig['access_menu']['id_menu']['id_menu'])
                {
                    $flat=true;
                    break;
                }
